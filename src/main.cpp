        ...
        if (event == "telemetry") {
          ...
          */
          ...
          for (size_t i = 0; i < ptsx.size(); ++i) {
            eptsx[i] = (ptsx.at(i) - px) * cos(psi) + (ptsy.at(i) - py) * sin(psi);
            eptsy[i] = (ptsx.at(i) - px) * -sin(psi) + (ptsy.at(i) - py) * cos(psi);
          }
          ...
          std::cout << "state: " << state << "  steer: " << steer_value << "  throttle: " << throttle_value << std::endl;
          ...
          size_t N = 8;
          vector<double> mpc_x_vals(vars.begin() + 2, vars.begin() + 2 + N);
          vector<double> mpc_y_vals(vars.begin() + 2 + N , vars.begin() + 2 + N + N);
...
//          for (size_t i = 0; i < ptsx.size(); ++i) {
//            next_x_vals.at(i) = (ptsx.at(i) - px) * cos(psi) + (ptsy.at(i) - py) * sin(psi);
//            next_y_vals.at(i) = (ptsx.at(i) - px) * -sin(psi) + (ptsy.at(i) - py) * cos(psi);
//          }

          for (int i = -1; i < 14; ++i) {
            next_x_vals.at(i + 1) = i * 4;
            next_y_vals.at(i + 1) = polyeval(coeffs, i * 4);
          }
